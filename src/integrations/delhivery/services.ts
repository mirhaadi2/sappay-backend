...
export const checkPincodeServiceability = async (
  ...
  try {
    ...
    console.error(
      "Delhivery Service Error [checkPincode]:",
...
export const createShipment = async (shipmentData: any): Promise<any> => {
  try {
    const products = Array.isArray(shipmentData.products)
      ...
      shipments: [
        {
          ...
          cod_amount: shipmentData.isPrepaid
            ? 0
            : parseFloat(shipmentData.totalAmount),
    ...
    if (response.data && response.data.packages && response.data.packages[0]) {
      console.log(
        "Detailed Remarks from Delhivery:",
        ...
        const shipment = await Shipment.create(
          {
            ...
            status: "CREATED",
            totalPackages:
              response?.data?.package_count || response?.data?.packages.length,
    ...
    if (error.message && error.message.includes("JSON")) {
      throw new Error(
        `Delhivery API returned invalid JSON response. Status: ${error.response?.status}, Response: ${error.response?.data}`,
      );
    }
...
export const trackShipmentService = async (waybill: string): Promise<any> => {
  try {
    ...
    console.error(
      "Delhivery Service Error [trackShipment]:",
      error.response?.data || error.message,
    );
...
export const editShipment = async (shipmentData: any): Promise<any> => {
  try {
    ...
    console.error(
      "Delhivery Service Error [editShipment]:",
      error.response?.data || error.message,
    );
...
export const cancelShipment = async (waybill: string): Promise<any> => {
  try {
    ...
    console.error(
      "Delhivery Service Error [cancelShipment]:",
      error.response?.data || error.message,
    );
...
export const updateEwaybill = async (
  waybill: string,
  data: any,
): Promise<any> => {
  try {
    const response = await delhiveryClient.put(
    ...
    console.error(
      "Delhivery Service Error [updateEwaybill]:",
      error.response?.data || error.message,
    );
...
export const calculateCharges = async (params: any): Promise<any> => {
  try {
    ...
    console.error(
      "Delhivery Service Error [calculateCharges]:",
      error.response?.data || error.message,
    );
  ...
  try {
    ...
    if (pdf) {
      ...
      const looksLikeJson =
        ...
        buffer.slice(0, 1).toString() === "{";
      if (looksLikeJson) {
        const jsonText = buffer.toString("utf8");
        const json = JSON.parse(jsonText);
        const pdfLink =
          json?.pdf_download_link || json?.packages?.[0]?.pdf_download_link;
        ...
        console.error(
          "Delhivery Service Error [generatePackingSlip] JSON payload had no pdf_download_link:",
          json,
        );
        throw new Error(
          "Packing slip API returned JSON without a PDF download link",
        );
      }
    ...
    console.error(
      "Delhivery Service Error [generatePackingSlip]:",
      error.response?.data || error.message,
    );
...
export const createPickupRequest = async (pickupData: any): Promise<any> => {
  try {
    ...
    console.error(
      "Delhivery Service Error [createPickupRequest]:",
      error.response?.data || error.message,
    );
