...
export const registerUser = async (email: string, password: string) => {
    return withTransaction(async (transaction) => {
        ...
        const hashed = await hashPassword(password);
        const user = await createUser(
            { email, password: hashed, role: 'D2C_CUSTOMER' },
            transaction,
        );
...
export const verifyOtpForLogin = async (
    ...
    return withTransaction(async (transaction) => {
        ...
        console.log(user, 'user after OTP verification');

        const token = signJwt({
            sub: user?.id || user?.dataValues?.id,
            email: user.email || user?.dataValues?.email || '',
            role: (user.role || user?.dataValues?.role || '') as any,
        });
        ...
        return {
            user: {
                id: user?.id || user?.dataValues?.id,
                name: user?.name || user?.dataValues?.name || '',
                email: (user.email || user?.dataValues?.email || '') as string,
                phone: user.phone || user?.dataValues?.phone || '',
                role: (user.role || user?.dataValues?.role || '') as any,
            },
            token,
