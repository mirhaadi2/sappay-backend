...
router.post('/validate', async (req, res) => {
  try {
    ...
    const result = await validateCoupon(couponCode, userId || null, cartItems || [], Number(subtotal || 0));
...
router.post('/apply', async (req, res) => {
  try {
    ...
    if (!validation.valid || !validation.coupon) return res.status(400).json({ success: false, message: validation.message });
