...
export const findCustomerByContact = async (
    ...
    const addresses = await findCustomerAddresses(customer?.id || customer?.dataValues?.id);
    const orderCount = await countCustomerOrdersRecord(customer?.id || customer?.dataValues?.id);
