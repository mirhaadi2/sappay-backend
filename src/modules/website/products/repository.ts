...
export const createProduct = async (data: any, transaction?: any) => {
    ...
    logger.info('Product created', {
        productId: product?.dataValues?.id || product.id,
        name: data.name,
    });
    return product;
};
...
export const findProductById = async (id: string, transaction?: Transaction) => {
  ...
  `;
    ...
    const [resolvedImages] = await Promise.all([resolveR2Urls(products.images || [])]);
...
const fastFormatProducts = async (products: any[]) => {
    ...
    const firstImageKeys = products.map((p) => p?.images?.[0]).filter(Boolean);
...
export const findAllProductsCatalog = async (filters: any) => {
    const { limit = 100, offset = 0, search, categoryId, status = 'ACTIVE' } = filters;
    const where: any = {};
...
export const createProductVariants = async (
    ...
    const sanitized = variants
        ...
        .map((v) => ({
            ...
            discountedPercent:
                v.discountedPercent !== undefined ? Number(v.discountedPercent) : undefined,
...
export const createSellerProduct = async (data: any) => {
    ...
    try {
        ...
        await transaction.rollback();
        logger.error('Error creating seller product', {
            sellerId: data.sellerId,
