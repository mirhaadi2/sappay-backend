...
} from '../../../integrations/razorpay/services';
...
export const placeOrderHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const result: any = await placeOrderService(
            ...
            req.body,
            guestData
                ? { contact: guestData?.contact, contactType: guestData.contactType }
                : undefined,
        );
        res.status(201).json({
            success: true,
        ...
        try {
            ...
            logger.info('Sales team notification sent', {
                orderId: result.id,
        ...
        if (!isPrepaid && customer?.email) {
            try {
                ...
                logger.error('Failed to send sales team notification', {
                    error,
                    orderId: result.id,
                });
            }
...
export const confirmPaymentHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const { razorpayPaymentId, razorpayOrderId, razorpaySignature } =
            req.body as RazorpayPaymentConfirmationPayload;
    ...
    try {
        ...
        if (event.event === 'payment.captured') {
            try {
                ...
                if (
                    webhookError instanceof AppError &&
                    webhookError.name === 'BadRequest' &&
...
export const getSellerOrdersHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
...
export const updateItemStatusHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
