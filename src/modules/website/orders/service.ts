...
const isPrepaidPaymentMethod = (paymentMethod: string) =>
  paymentMethod !== "cod";
...
const createPaymentGatewayOrder = async (
  ...
  if (provider === "razorpay") {
    const auth = Buffer.from(
      `${config.payment.apiKey}:${config.payment.apiSecret}`,
    ).toString("base64");
  ...
  return withTransaction(async (transaction) => {
    ...
    if (!customerId && !guestData) {
      throw new AppError(
        "BadRequest",
        400,
        "Customer or guest information is required",
      );
    }
    ...
    if (!customerId && guestData) {
      ...
      const guestEmail =
        guestData.contactType === "email" ? guestData.contact : undefined;
      const guestPhone =
        guestData.contactType === "phone" ? guestData.contact : undefined;
      const guestWhatsapp =
      ...
      if (existingCustomer) {
        finalCustomerId =
          existingCustomer?.id || existingCustomer?.dataValues?.id;
    ...
    const guestEmail =
      guestData && guestData.contactType === "email"
        ? guestData.contact
        : undefined;
    const guestPhone =
      guestData && guestData.contactType === "phone"
        ? guestData.contact
        : undefined;
...
export const confirmPaymentService = async (orderId: string) => {
  ...
  try {
    ...
    logger.error(
      "Failed to send sales team notification for confirmed payment",
      { error, orderId: result.id },
    );
  }
  ...
  if (customer?.email) {
    try {
      ...
      logger.error(
        "Failed to send customer confirmation for confirmed payment",
        { error, orderId: result.id, customerEmail: customer.email },
      );
    }
  }
...
export const getCustomerOrderService = async (
  ...
  const order: any = await findCustomerOrder(
    customerId,
    orderId,
    customerEmail,
  );
