...
import { findAdminByEmailRecord, findAdminByIdRecord, findStaffByEmailRecord } from './repository';
 ...
 * Find admin by email
 */
export const findAdminByEmail = async (email: string): Promise<any> => {
    try {
        return await findAdminByEmailRecord(email);
    } catch (error) {
 ...
 * Find admin by ID
 */
export const findAdminById = async (id: string): Promise<any> => {
    try {
        return await findAdminByIdRecord(id);
    } catch (error) {
    ...
    try {
        ...
        if (admin) {
            ...
            if (admin.status !== 'active') {
                logger.warn('Login attempt for inactive admin', {
                    adminId: admin.id,
                    status: admin.status,
                });
                throw new AppError(
                    'UnauthorizedError',
                    401,
                    `Account is ${admin.status}. Please contact support.`,
                );
            }
        ...
        if (staff) {
            ...
            logger.info('Staff login successful via admin portal', {
                staffId: staff.id,
                email,
 ...
 */
const findStaffByEmail = async (email: string): Promise<any> => {
    try {
