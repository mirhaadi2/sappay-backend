...
import { getCloudflareR2SignedUrl as getR2SignedUrl } from '../../../infrastructure/storage/cloudflare';
...
const transformVariant = (variant: any): AdminProductVariantDetail => ({
    ...
    discountedPercent:
        variant.discountedPercent !== undefined ? Number(variant.discountedPercent) : undefined,
...
const transformSellerOffering = (offering: any): AdminProductSellerOffering => ({
    ...
    sellerCommissionRate: offering.sellerCommissionRate
        ? Number(offering.sellerCommissionRate)
        : undefined,
    ...
    lastRestockedAt: offering.lastRestockedAt
        ? new Date(offering.lastRestockedAt).toISOString()
        : undefined,
});
...
export async function transformProductToAdmin(row: ProductRow): Promise<AdminProduct> {
    ...
    const transformedSellerOfferings: AdminProductSellerOffering[] = Array.isArray(
        row.sellerOfferings,
    )
    ...
    return {
        ...
        sellerOfferings:
            transformedSellerOfferings.length > 0 ? transformedSellerOfferings : undefined,
 ...
 */
export function resolveSortColumn(sortBy: string = 'created_at'): {
    column: string;
    order: string;
} {
