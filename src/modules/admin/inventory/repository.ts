...
export const listInventory = async (params: AdminInventoryQuery) => {
    try {
        const { page = 1, limit = 20, productId, sellerId, lowStock = false, search } = params;
        ...
        if (search) {
            whereConditions.push(
                '(p.name ILIKE :search OR s.business_name ILIKE :search OR sp.seller_sku ILIKE :search)',
            );
...
export const updateInventory = async (
    inventoryId: string,
    updates: AdminInventoryUpdateInput,
    ...
    if (finalAvailableStock !== undefined)
        updateData.availableStock = Math.max(0, finalAvailableStock);
        ...
        await InventoryHistory.create(
            {
                ...
                type: 'ADJUSTMENT',
                quantity: totalStock
                    ? totalStock - parseNumber(inventory?.dataValues?.totalStock)
                    : 0,
    ...
    return await Inventory.findByPk(inventoryId, { transaction });
};

export const addStock = async (
    inventoryId: string,
    input: AdminAddStockInput,
    transaction?: Transaction,
) => {
    const inventory = await Inventory.findByPk(inventoryId, { transaction });
    if (!inventory) {
...
export const removeStock = async (
    ...
    const newAvailableStock = Math.max(
        0,
        parseNumber(inventory?.dataValues?.availableStock) - quantity,
    );
