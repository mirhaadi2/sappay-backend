...
export const requireAllPermissions = (permissions: string[]) => {
    return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        try {
            ...
            const hasAllPermissions = permissions.every((perm) => staffPermissions.includes(perm));

            if (!hasAllPermissions) {
                const missing = permissions.filter((perm) => !staffPermissions.includes(perm));
                return res.status(403).json({
...
export const requireAnyPermission = (permissions: string[]) => {
    return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        try {
            ...
            const hasAnyPermission = permissions.some((perm) => staffPermissions.includes(perm));
