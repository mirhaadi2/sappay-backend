...
export const checkInventoryByProductIdService = async (
    ...
    const productVariant = await getProductVariantWeightInfo(productVariantId, transaction);
...
export const checkAvailabilityService = async (
    ...
    const productVariant = await getProductVariantWeightInfo(productVariantId);
    if (!productVariant) return false;
