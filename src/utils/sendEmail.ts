...
export const sendEmail = async (options: SendEmailOptions) => {
  const mailOptions = {
    from: options.from || `"Sappay Support" <${config.email.smtpHost}>`,
    to: options.to,
...
export const sendOtpToEmail = async (email: string, code: string) => {
  return sendEmail({
    to: email,
    subject: "Your Sappay Verification Code",
    html: `
      <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 5px;">
        <h2 style="color: #4b3832;">Verify your account</h2>
        <p>Your OTP for Sappay registration is:</p>
...
export const sendPasswordResetEmail = async (email: string, resetLink: string) => {
  return sendEmail({
    to: email,
    subject: "Reset Your Sappay Password",
    html: `
      <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 5px;">
...
export const sendWelcomeEmail = async (email: string, name: string) => {
  return sendEmail({
    to: email,
    subject: "Welcome to Sappay!",
    html: `
      <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 5px;">
        <h2 style="color: #4b3832;">Welcome to Sappay, ${name}!</h2>
        ...
        <hr style="border: none; border-top: 1px solid #eee; margin: 20px 0;" />
        <p style="font-size: 12px; color: #999;">Questions? Contact us at support@sappay.com</p>
      </div>
    `,
  });
};
